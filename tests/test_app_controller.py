...
class Base(unittest.TestCase):
    ...
    def controller(self, executor, **kwargs):
        ...
        # Store보다 늦게 등록하므로 먼저 돈다. 단언이 중간에 실패해도 새 시도를 막고 worker가 돌아온 뒤에 Store를
        # 닫는다 — 닫힌 DB에 쓰는 worker의 traceback이 다음 시험의 출력에 섞이지 않는다.
        self.addCleanup(self._stop, ctl)
        return ctl

    def _stop(self, ctl):
        self.assertTrue(ctl.shutdown(SLOW_RUNNER_TIMEOUT), "controller worker still running at teardown")
