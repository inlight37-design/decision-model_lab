...
This is a research and architecture repository with offline synthetic checks, not a working multi-provider orchestrator. It is the one rule file for every agent: Claude Code loads it through [CLAUDE.md](CLAUDE.md), Codex reads it directly, and any other assistant should be given it.
...
- **Finish:** update `NEXT-SESSION.md` in the same PR (its section layout is checked by CI), fill the [PR template](.github/pull_request_template.md), and do not call anything passing before you have seen CI green. The user merges.
...
- D10-D18 map to F01–F29 in [new sources](docs/architecture/v0.4/sources.json); D01-D09 and E01-E31 remain in v0.3. Distinguish primary-source inspection, vendor reports, abstract-only evidence and project proposals. Add a registry entry only after reading the primary source yourself; review-local IDs such as `R01` are not registry entries.
