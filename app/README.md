...
- **원본 앱(수동).** ChatGPT·Claude·Antigravity 앱에서 사용자가 직접 돌린다.
  ...
  3. controller가 답을 받기 전에 확인하는 것: 입력 sha256이 이 실행의 것과 같은지, 늦게 온 답이나 중복 제출이 아닌지, 빈 답이 아닌지.
     - **주의:** 화면은 그 카드가 속한 실행의 sha256을 자동으로 넣어 보낸다. 그래서 화면에서는 sha256 검사가 사실상 늘 통과한다. 막는 것은 화면을 거치지 않은 요청의 불일치뿐이다. **다른 실행의 답을 이 카드에 붙여 넣는 것도, 원본 앱에 정말 이 질문을 넣었는지도 확인하지 못한다** — 사용자의 확인에 기댄다(인계 4절 K21, A1 리뷰 A1-08).
