...
| [roles.py](roles.py) | A 단계 지원 배치 검사·역할 고정·공개 투영에서 작업 목록 계산 |
...
출력된 `http://127.0.0.1:8765/#token=...` 주소를 연다. **홈 → 새 작업 → 역할판 → 보낼 입력 확인 → 시작 → 기존 결과 화면**으로 이어진다. 홈 위의 **내 차례**에는 원본 앱 답 붙여넣기·축소 여부·공개 뒤 판단을 기다리는 작업이 모인다. 작업에는 시간순 실행 타임라인과 **이 작업에 새 실행**이 있고, 실행 카드를 누르면 원래 결과 화면이 열린다. **판단 완료**를 누르면 사람의 차례가 끝난다. 새로고침·서버 재시작 후에도 작업·역할·판단 완료가 남는다.

카드를 끌어다 놓거나 **카드 → 칸**을 누른다(Tab·Enter·Space도 가능). 빈 슈퍼바이저·오케스트레이터는 **나**다. A 단계에서 격리 팀원은 기존 참여자·정족수 정책, 오케스트레이터는 공개 뒤 누르는 기존 합성에 연결된다. 오케스트레이터가 나이면 합성을 부르지 않고 원문 대조표를 보여 준다. 슈퍼바이저 모델(D), 일반 팀원(C), 같은 provider 두 장, 수동 앱 오케스트레이터는 놓을 수 있지만 시작 전에 이유를 보이고 서버도 거절한다. 질문은 **원문**이 기본이며 **다듬기**는 다음 단계 안내만 보이고 선택할 수 없다. 원본 앱 카드는 격리 칸에서도 **독립 미확인**이다.

시작 전 화면은 질문과 실제 전달문 전문(수동 참여자의 표식 포함), 공통 자료 목록·해시, 고정 역할, 정족수, 호출 상한을 보여 준다. 확인 중 입력을 바꾸면 다시 확인해야 한다. 미리보기는 원장을 쓰거나 호출하지 않으며, 같은 미리보기로 시작을 반복해도 실행을 중복 생성하지 않는다. A 단계는 진행 중·종료 미확인 실행이 있을 때 새 역할판 실행을 거절한다. 모의 CLI는 실제 공급자를 부르지 않는다. 원본 앱 참여는 질문 복사→답 붙여넣기이며 화면 자동화는 없다. 실제 CLI의 준비 조회·계획·허가·봉인 관문은 그대로다. A 화면의 실제 CLI 확인은 해당 PC 세션이 맡는다.
...
스키마 **8**은 `tasks`와 실행의 `task_id`·`role_config`를 추가한다. 작업 생성(또는 기존 작업 연결), 실행, 역할·입력 모드·참여자 명세·자료가 같은 거래에서 고정된다. 시작한 실행의 역할 변경 API는 없고 변경하려면 새 실행이다. 기존 원장의 실행은 **실행마다 작업 하나**로 이전하며, 제목은 질문 앞부분이다. 기존 실행의 합성 선택은 보존하고 새 역할판 실행의 합성자는 시작 때 고정한다. 홈·타임라인·내 차례는 `controller.view()`의 공개 투영만 쓰며, 초안·결과 메타데이터를 별도로 읽지 않는다. 작업과 역할은 서버 원장에 저장하며 브라우저 저장소에는 넣지 않는다.

구형 원장을 열면 원장 잠금을 잡고 이전 거래를 시작하기 **전에** SQLite backup으로 옆에 `journal.db.v<이전 판>-<고유 ID>.bak` 사본을 완성한다. 사본 작성이 실패하면 이전도 시작하지 않는다. 사본에는 초안이 있으므로 원장과 같은 비공개 폴더에 두며, 새 스키마를 옛 코드로 여는 하향 이전은 거절한다.
...
결정 보고는 `GET /api/runs/<run_id>/decision-report`의 `a1-decision-report/4`이며 합성의 `schema`(모의 `a1-mock-synthesis/1`, 실제 `a1-model-synthesis/1`)로 종류를 가른다. `synthesis`는 가장 최근에 끝난 합성(모의 또는 실제)이고, `model_syntheses`는 실제 합성 시도 전부를 순서대로 싣는다(시도 ID·상태·결과 — 실패면 이유와 검사 실패한 원문). 3판의 `failed_model_synthesis`는 이 목록으로 바뀌었다. 합성이 하나도 없으면 409다. 화면은 가장 최근 결과를 보이고, 둘 이상 부른 실행은 시도 목록을 접어 보인다. Q4는 A 결정 우선이 기본이고, B 대조표 우선은 같은 결과의 표시 순서만 바꾸는 전환이다([인계](../NEXT-SESSION.md) 2절 25).
