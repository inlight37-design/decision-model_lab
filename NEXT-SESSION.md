...
최종 갱신 **2026-09-25** · 작성 세션: claude(Claude 데스크톱 앱, `aux-pc`의 Windows·WSL) · 브랜치 `claude/codex-apps-off-20260924` · 기준 main `6875fffe99804ea410e0e9ff547dfb6d496a744e`(PR #44 병합).
...
3. 이 판을 쓴 claude 세션은 `aux-pc`의 Windows와 WSL `aux-pc-wsl`에서 시험·변이 시험과 실제 호출(B: Codex 1·Claude 1, C: Claude 1, D: Codex 6·Claude 9, E: Codex 1)을 했다. E에서는 실제 로그인 폴더를 연결한 모델 없는 Codex app-server 조회도 했다. WSL의 Codex 0.156.1·Claude Code 2.1.280이 구독 로그인으로 동작했고, 띄운 서버는 끝냈다. 인증 폴더 내용은 읽지 않았다.
...
**진행 중:** `claude/codex-apps-off-20260924` — 4절 E의 첫 단계(Codex 연결 앱 끄기와 K46 재관측). 모델 호출 1회(Codex). 병합 직전 커밋에서 이 줄을 지운다.

- **마지막 병합:** [PR #44](https://github.com/inlight37-design/decision-model_lab/pull/44)(`claude/comparison-pilot-20260924`) — 4절 D(작은 비교: 사전 등록과 결과). 모델 호출 15회. claude 세션이 병합했다(2절 8). [결과](docs/experiments/2026-09-24-comparison-pilot/RESULTS.md).
