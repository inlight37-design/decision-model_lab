...
6. **사용자에게 받을 것(3절):** Claude 문맥 칸을 `unknown`으로 바꿀지(리뷰 R04), K46 확인 호출(Codex 1회)의 승인 카드, 두 CLI의 문맥 판정을 푸는 방법(C3), PR #8(tmux 조사, 병합됨)의 TM 항목을 작업으로 받을지. 답을 받기 전에는 진행하지 않는다.
...
- **마지막 병합:** 병합된 브랜치를 지우는 Actions 작업(`claude/cleanup-merge-branches-il1srl`의 두 번째 PR, 모델 호출 없음). 웹 세션이 지우는 방법 셋 중 사용자가 고른 것이다(2026-09-24).
  - [`prune-merged-branches`](.github/workflows/prune-merged-branches.yml)는 손으로만 돈다. main에 모두 들어간 원격 브랜치만 지우고, `dry_run`이면 목록만 보인다. GitHub의 토큰으로 지우므로 자기 브랜치에만 push할 수 있는 웹 세션도 쓸 수 있고, 사용자는 휴대폰의 GitHub 웹에서도 돌릴 수 있다(Actions 탭 → prune-merged-branches → Run workflow).
  - 병합한 뒤 돌려서 쌓인 병합 브랜치를 지운다. 남아 있으면 다시 돌린다 — 두 번째 실행은 지울 것이 없으면 그대로 끝난다.
- **그 앞:** [PR #16](https://github.com/inlight37-design/decision-model_lab/pull/16) 브랜치 정리와 PR #15 후속(모델 호출 없음). 사용자 지시("여기 브랜치가 왜이렇게 많은거지 확인하고 좀 정리해서 병합좀해줘")로 claude 웹 세션이 했다.
  ...
  - 원격 브랜치가 많았던 까닭: 세션마다 주제별 브랜치를 만들었고, 병합한 뒤 지우는 규칙이 없었다. 남은 브랜치는 모두 main에 들어가 있었다(이 세션이 하나씩 확인했다). [협업 규칙](docs/COLLABORATION.md) 5절에 "병합한 쪽이 지운다"와 한꺼번에 지우는 명령을 더했다. 이 세션은 자기 브랜치 밖으로 push할 수 없어(403) 지우지 못했다 — 위 Actions 작업을 더한 까닭이다.
...
사용자의 판단을 기다리는 것:
- **GitHub 설정 "Automatically delete head branches"(권장, 저장소 주인만 켤 수 있다).** [저장소 설정 첫 화면](https://github.com/inlight37-design/decision-model_lab/settings)의 Pull Requests 부분에 있다(휴대폰에서는 GitHub 앱이 아니라 브라우저로). 켜면 GitHub에서 병합한 PR의 브랜치는 저절로 지워진다 — 로컬에서 병합해 main에 push한 경우에도 지워지는지는 확인하지 않았다. 그래도 남는 병합 브랜치는 위 Actions 작업이 지운다.
