...
최종 갱신 **2026-09-25** · 작성 세션: claude(`aux-pc` Windows 데스크톱 앱, 모델 CLI 호출 없음) · 브랜치 `claude/merge-56-20260925`([PR #57](https://github.com/inlight37-design/decision-model_lab/pull/57)) · 기준 main `6e69346`(PR #55 병합). ChatGPT 웹 세션의 #56(여러 AI 작업 방식 후속 검토)을 출처까지 다시 읽고, 그 head 위에 검토 기록을 더해 병합한다. 앞 판은 #56의 ChatGPT 웹 세션(GitHub·공식 웹 문서만 봄), 그 앞은 claude(`aux-pc`)의 #55 조사이며 앞선 구현·실측은 아래 기록을 따른다.
...
- 긴 자료 실험의 후속 후보: 전체 1 MiB를 실제로 한 번 보고 앱의 자료 한도를 참여자가 제한 시간 안에 쓸 수 있는 크기로 맞출지 정한다. 뜻을 읽어야 찾는 사실(눈에 띄는 표식 없음)은 시험하지 않았다. 긴 자료를 순서대로 읽는 Claude 참여자는 구독 사용량을 많이 쓴다(짧은 자료 실험의 API 환산 추정액 대비 약 33배).
