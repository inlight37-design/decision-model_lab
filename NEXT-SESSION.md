...
| ID | 질문 | 상태 |
|---|---|---|
| Q1 | 어댑터 전송 | **확정 — exec 우선**(2026-09-23, 사용자가 판단을 claude 세션에 맡김). 세 CLI 모두 help에 ACP가 없고, 비대화형 실행이 세 CLI 모두 구독으로 동작했다(aux-pc V04-01). ACP는 adapter 계층의 선택지로 남긴다 |
...
13. **유료 API로 전환하지 않는다. 모델은 붙였다 뗐다 하는 구조다.** 원하는 모델을 adapter로 붙여 구성을 늘리고 줄인다. 구독 경로가 닫히거나 한도를 다 쓰면 그 provider만 뺀다. 빠진 자리를 다른 모델로 조용히 채우지 않고 구성이 줄었음을 표시한다(D18). (2026-09-23)
