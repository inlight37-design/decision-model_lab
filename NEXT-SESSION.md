...
최종 갱신 **2026-09-25** · 작성 세션: claude(`aux-pc` Windows 데스크톱 앱, 모델 호출 0 — 정리 1단계: 인계·규칙 문서 줄이기, CI 실행 방식) · 브랜치 `claude/handoff-diet-20260925` · 기준 main `2450caf`(PR #74 병합).

이 파일은 **지금 상태와 다음 일만** 담는다. 끝난 일의 경위는 PR·git 이력과 날짜가 붙은 기록(`docs/reviews/`, `docs/experiments/`)에 있고, 이번에 줄이기 전 판은 [보관본](docs/handoff/2026-09-25-before-handoff-diet.md)이다. **3절에는 진행 중인 일과 마지막 병합 하나만 둔다** — 새 PR은 "마지막 병합"을 자기 것으로 바꾸고 이전 것은 지운다(PR 목록이 이력이다). 2절·5절은 사용자 결정과 금지 사항이라 줄이지 않았다.
...
| 한계·남은 일 | 못 고치는 것, 해야 할 일의 우선순위, 조사 거리를 한 장에 모았다(외부 검토 대기, 카드 #73) | [검토 요청서](docs/reviews/2026-09-25-review-request/README.md) |
...
23. **정리하고, 쓸모없는 것이 다시 쌓이지 않게 한다.** 제안한 순서대로 정리한다(3절). 쓸모없는 도구는 지워도 된다. 합성 제약 — 실행마다 실제 합성 한 번, 합성자는 그 실행의 참여자 provider만 — 은 없앤다. 호출 상한·동시 하나·종료 미확인 규칙은 그대로 둔다. CI 실행 방식은 claude 세션에 판단을 맡겼다. 정리한 뒤 쌓임을 막는 원칙과 규칙을 정하되, 정하기 전에 충분히 고민한다. (사용자, 2026-09-25)

## 3. 진행 중인 작업

**진행 중: 정리(2절 23).** 순서 — ① 인계·규칙 문서 줄이기와 CI 실행 방식(이 PR) ② 쓸모없는 도구 은퇴와 로컬 정리 ③ 합성 제약 제거 ④ 헤드리스 실행 명령 ⑤ 재관측 자동화와 기기 묶기(카드 #71, 2026-10-25 전) ⑥ 쌓임 방지 원칙(검토를 받고 정한다). 근거는 [검토 요청서](docs/reviews/2026-09-25-review-request/README.md)의 B·C절이다.
...
1. 3절 정리 순서의 다음 단계.
2. 보드의 `status: ready` 카드. 상태는 보드가 기준이고 우선순위 제안은 [검토 요청서](docs/reviews/2026-09-25-review-request/README.md) B절이다 — 지금 준비된 것은 #70(고립 surrogate로 실행이 멈춤), #71(관측 기록을 기기와 묶기), #64(Codex 참여자의 계정 플러그인), #72(L1 실험), #61(자료 합계 1 MiB), #73(외부 검토).
