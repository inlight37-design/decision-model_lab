...
최종 갱신 **2026-09-25** · 작성 세션: claude(`aux-pc` Windows 데스크톱 앱, WSL에서 모델 없는 조회만) · 브랜치 `claude/plugin-surface-20260925`([PR #65](https://github.com/inlight37-design/decision-model_lab/pull/65)) · 기준 main `a3e4672`(PR #63 병합). 사용자가 ChatGPT에 GitHub 플러그인을 붙여 카드 #59를 끝냈고, 계정의 플러그인이 앱의 Codex 참여자에게 닿지 않는지 모델 없이 확인했다. 앞 판은 같은 세션의 #63(카드 #58)·#62(시범 보드)·#57(#56 검토·병합)이고, 그 앞은 #56의 ChatGPT 웹 세션과 claude(`aux-pc`)의 #55 조사다. 앞선 구현·실측은 아래 기록을 따른다.
