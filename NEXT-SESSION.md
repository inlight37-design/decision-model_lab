...
이 파일은 **지금 상태와 다음 일만** 담는다. 끝난 일의 경위는 PR·git 이력과 날짜가 붙은 기록(`docs/reviews/`, `docs/experiments/`)에 있고, 이번에 줄이기 전 판은 [보관본](docs/handoff/2026-09-25-before-handoff-diet.md)이다. **3절에는 진행 중인 일과 "이 판을 들인 PR" 하나만 둔다** — 새 PR은 그 줄을 자기 PR로 바꾼다. 병합 전 브랜치에서도 병합 뒤 main에서도 맞는 말만 쓰고, "병합했다"고 미리 적지 않는다(외부 검토 R07). 이력은 PR 목록이다. 2절·5절은 사용자 결정과 금지 사항이라 줄이지 않았다.
