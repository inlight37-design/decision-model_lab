...
최종 갱신 **2026-09-25** · 작성 세션: claude(`aux-pc` Windows 데스크톱 앱, WSL의 Codex 8·Claude 5 — 카드 #60 D 후속 비교) · 브랜치 `claude/d-followup-20260925`([PR #67](https://github.com/inlight37-design/decision-model_lab/pull/67)) · 기준 main `a3e4672`(PR #63 병합), 도중에 `ff1b9de`(PR #65)를 병합. 작업 카드 시범의 #60을 카드와 인계만 보고 이어받아 사전 등록 → 실행 → blind 채점으로 끝냈다. 앞 판은 같은 시간에 다른 claude 세션이 쓴 #65(카드 #59, 계정 플러그인 조회)이고, 그 앞은 #63(카드 #58)·#62(시범 보드)·#57(#56 검토·병합), #56의 ChatGPT 웹 세션과 claude(`aux-pc`)의 #55 조사다. 앞선 구현·실측은 아래 기록을 따른다.
...
3. 이전 판을 쓴 claude 세션들은 `aux-pc`의 Windows와 WSL `aux-pc-wsl`에서 시험·변이 시험과 실제 호출(B: Codex 1·Claude 1, C: Claude 1, D: Codex 6·Claude 9, E: Codex 1, E2: Codex 4·Claude 2, strict 실행·취소: Codex 3·Claude 1, 자료 실험: Codex 1·Claude 1, 긴 자료 실험: Codex 1·Claude 1, 카드 #58 교차검토: Codex 1, 카드 #60 D 후속 비교: Codex 8·Claude 5, 카드 #66 교차검토: Codex 1)을 했다. E·E2에서는 실제 로그인 폴더를 연결한 모델 없는 조회(Codex app-server, `codex debug prompt-input`, 네트워크 없는 `codex sandbox`·exec)도 했다. 카드 #59 뒤에는 같은 방식으로 계정 플러그인이 참여자에게 닿는지 조회했다([플러그인 기록](docs/reviews/2026-09-25-plugin-surface/README.md)). WSL의 Codex 0.156.1·Claude Code 2.1.280이 구독 로그인으로 동작했고, 띄운 서버는 끝냈다. 인증 폴더 내용은 읽지 않았다. #46·#47·#52의 ChatGPT 웹 세션은 기록과 코드를 검토하고 오프라인 검사(#52는 GitHub Actions)를 했을 뿐 사용자 PC를 보지 않았다.
...
| 원장·서버 | 이전 원장 보존. #39 병렬 원장은 전체 2/2 소진. B·C 원장 `~/.local/state/dml-live-b-20260924`(aux-pc-wsl)은 전체 3/3·Codex 1/1·Claude 2/2로 소진(참여자 2 + 합성 1). D 원장은 [결과](docs/experiments/2026-09-24-comparison-pilot/RESULTS.md)에 적은 대로 소진. E의 K46 상태 폴더 `~/.local/state/dml-observe-e-20260924`는 Codex 1/1 소진. E2의 상태 폴더 `~/.local/state/dml-observe-e2-20260925`는 두 승인 창(Claude 2·Codex 1, Codex 3)을 모두 썼다. strict 원장 `~/.local/state/dml-live-strict-20260925`는 3/3(Codex 2·Claude 1), 취소 원장 `~/.local/state/dml-live-cancel-20260925`는 1/1, 자료 실험 원장 `~/.local/state/dml-live-sources-20260925`는 2/2 소진, 긴 자료 실험 원장 `~/.local/state/dml-live-long-20260925`도 2/2 소진(Codex 1·Claude 1). D 후속 비교의 원장 여섯(`~/.local/state/dml-dfollow-20260925/` 아래 `t1-px`·`t2-px`·`t4-s-codex`·`t4-s-claude`·`t4-px`·`grade`)은 모두 상한까지 썼다(합계 Codex 8·Claude 5). 이전 claude 세션들은 자신이 띄운 서버 종료를 확인했다(포트 닫힘·잠금 해제). #50·#51의 세션은 띄운 서버를 모두 끈 뒤 포트 닫힘과 남은 프로세스 없음을 확인했다 |
...
- **마지막 병합:** [PR #67](https://github.com/inlight37-design/decision-model_lab/pull/67)(`claude/d-followup-20260925`) — [카드 #60](https://github.com/inlight37-design/decision-model_lab/issues/60) D 후속 비교. 카드와 인계만 보고 이어받았다(사용자 재설명 없음). 호출 전에 [사전 등록](docs/experiments/2026-09-25-d-followup/README.md)을 커밋하고(T1·T2는 D와 같은 질문 — 입력 digest 일치, T4는 여섯 조건이 충돌하는 백업 설계), 합성자를 Codex로 바꿔 부른 뒤, 이름을 가린 묶음을 이 세션이 먼저 채점해 커밋하고 나서 대응표를 열었다. [결과](docs/experiments/2026-09-25-d-followup/RESULTS.md): T1은 두 초안이 모두 2(Codex는 "0개"로 시작해 스스로 정정)라 L1을 판단할 수 없었고 Codex 합성은 형식 검사에 실패했다. T2에서 Codex 합성은 Codex 초안의 틀린 설명("첫 창이 빠진다")을 주장으로 받았다 — D의 Claude 합성과 같은 모양이다. T4는 다섯 답 모두 두 모순을 계산해 객관 항목에서 갈리지 않았고, 바꿀 조건을 골라 이유를 댄 것은 Claude의 두 답뿐이었다(합성은 목록만). Claude 채점자는 180초를 넘겨 채점 실행이 공개되지 않았고, 봉인된 Codex 채점은 사전 등록 밖 보조 분석으로만 실었다(읽기 오류 둘). 앱 코드 변경 없음(`app/README.md`의 남은 일 정정). 모델 호출: Codex 8·Claude 5(사전 등록 상한 그대로). 후속은 [카드 #66](https://github.com/inlight37-design/decision-model_lab/issues/66)(합성 실패 원문 보존·이름표 순서). [시범 기록](docs/experiments/2026-09-25-card-pilot/README.md)에 결과를 적었다. claude 세션이 병합했다(2절 8).
- **그 앞:** [PR #65](https://github.com/inlight37-design/decision-model_lab/pull/65)(`claude/plugin-surface-20260925`) — [카드 #59](https://github.com/inlight37-design/decision-model_lab/issues/59) 결과: ChatGPT에 GitHub 플러그인을 붙이면(`+`·`@GitHub`) 카드 이슈를 읽고 댓글을 단다. ChatGPT 데스크톱 앱의 컴퓨터 조작·브라우저 플러그인은 사용자 PC를 다루므로 그렇게 쓴 세션은 접근 범위에 적는다([AGENTS.md](AGENTS.md), [협업 규칙](docs/COLLABORATION.md) 1절). 계정에 켜진 원격 플러그인이 Codex 참여자에게 닿는지 모델 없이 조회했다 — 참여자 계획에서는 MCP 항목 0·호출 가능한 앱 0·기본 스킬뿐이고 모델 입력 렌더링도 E2와 같다([플러그인 기록](docs/reviews/2026-09-25-plugin-surface/README.md), 보강은 [카드 #64](https://github.com/inlight37-design/decision-model_lab/issues/64)). 코드 변경 없음, 모델 호출 0회. claude 세션이 병합했다(2절 8).
- **그 앞:** [PR #63](https://github.com/inlight37-design/decision-model_lab/pull/63)(`claude/check-cli-exit-20260925`) — [카드 #58](https://github.com/inlight37-design/decision-model_lab/issues/58): 준비 조회 거절의 종료 코드를 3으로 나눠 인자 오류(2)와 가른다(`app/server.py`, 시험, `app/README.md`). WSL의 Codex CLI가 읽기 전용으로 한 번 검토했고(42초), 지적 둘 — `--check-config` 거절 시험 누락, 보관된 옛 안내의 "거절 2" — 을 반영했다(옛 안내는 [보관 색인](docs/handoff/README.md)에 표시). 모델 호출: Codex 1. [시범 기록](docs/experiments/2026-09-25-card-pilot/README.md)에 결과를 적었다. claude 세션이 병합했다(2절 8).
...
V04-03(두 provider의 읽기 전용 독립 답변, [완료 조건](docs/architecture/v0.4/03-evaluation-and-roadmap.md))은 #39로 거의 채웠고 — 실제 두 참여자, 입력·초안 digest, 권한 거절, 종료 관측, 사용량 범위 — 마지막 조건이던 실제 CLI 중도 취소를 2026-09-25에 관측했다([기록](docs/reviews/2026-09-25-strict-live-run/README.md)). 아래 A–F는 그 뒤 진행한 구현·관측의 이력이다. D 후속 비교([카드 #60](https://github.com/inlight37-design/decision-model_lab/issues/60))는 [결과](docs/experiments/2026-09-25-d-followup/RESULTS.md)로 끝났다. [카드 #66](https://github.com/inlight37-design/decision-model_lab/issues/66)(실제 합성의 실패 원문 보존·이름표 순서 섞기, D 후속에서 나옴)도 끝났다. **다음은 보드의 `status: ready` 카드에서 고른다** — [카드 #64](https://github.com/inlight37-design/decision-model_lab/issues/64)(Codex 참여자의 계정 플러그인 막기), [카드 #61](https://github.com/inlight37-design/decision-model_lab/issues/61)(자료 합계 1 MiB, 낮음). 합성자가 제 계열의 틀린 초안을 이기고 맞는 쪽을 고르는지(L1)는 이제 이름표 순서가 섞이므로, 초안이 갈리는 과제를 사전 등록해 다시 볼 수 있다(아직 카드 없음). 긴 자료·파일 수 상한은 [사전 등록한 실험](docs/experiments/2026-09-25-long-sources/RESULTS.md)에서 두 참여자 모두 20개 파일과 약 250 KB 파일 둘의 앞·가운데·끝, 약 5,900자 한 줄 속 값을 모두 찾았다(Codex는 검색, Claude는 순서대로 읽기 — Claude가 제한 180초 중 104초를 썼다. 전체 1 MiB는 시험하지 않음). 짧은 자료 속 지시문은 [사전 등록한 실험](docs/experiments/2026-09-25-source-injection/RESULTS.md)에서 두 참여자 모두 따르지 않았다(Claude는 주입으로 지목하고 옮겨 적음, Codex는 언급하지 않음). strict 정책의 실제 두 참여자 실행은 같은 기록에서 독립 정족수를 채웠다. E2는 로그인 파일을 복사하지 않는 길(CLI 자신의 폴더는 그대로 쓰기, 모델의 명령만 막기, CLI 옵션으로 적재 끄기)로 끝냈으므로 인증 갱신·동시성 설계는 필요 없어졌다. 이미 끝난 A–D를 처음부터 다시 호출하지 않는다. 새 프레임워크·SDK·유료 API는 추가하지 않는다. 실제 호출은 모두 새 원장·provider별 상한·멈춤 조건으로 하고 결과를 기록한다(2절 22). 근거가 된 사실 확인은 [병합 기록 7절](docs/reviews/2026-09-24-merge-39/README.md)에 있다.
...
- 계정 플러그인(2026-09-25): 사용자 계정에 원격 플러그인이 설치·켜져 있지만 지금 Codex 참여자 계획에서는 닿지 않는다(MCP 항목 0·호출 가능한 앱 0·기본 스킬뿐, [플러그인 기록](docs/reviews/2026-09-25-plugin-surface/README.md)). 이 안전은 연결 앱 끄기에 기대므로, 스킬이나 자체 MCP 서버를 가진 플러그인이 설치되면 달라질 수 있다. 명시적 끄기나 실행 전 표면 검사는 [카드 #64](https://github.com/inlight37-design/decision-model_lab/issues/64)다. 플러그인을 새로 설치했다면 `probe.py`로 다시 본다.
...
- D 후속([결과](docs/experiments/2026-09-25-d-followup/RESULTS.md))에서 남은 것: L1(초안이 갈리고 합성자 계열 쪽이 틀린 사례 — 앱은 실행마다 합성을 한 번만 하므로 같은 초안에 두 합성자를 붙일 수 없다), 모델 채점(Claude는 답안 열 개를 180초 안에 채점하지 못했고 Codex 채점자는 읽기 오류 둘을 냈다 — 다시 쓰면 과제별로 나누고 근거를 원문과 대조). 다시 돌리려면 그 폴더의 `make_configs.py`·`drive.py`·`blind_pack.py`·`analyze.py`를 새 상태 폴더로 쓴다.
