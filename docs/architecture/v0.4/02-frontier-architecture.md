...
| D12 | claim/evidence 중심 합성, 미합의 허용, 투표≠검증 | F01/F04/F07/F12/F13/F20/F21/F22/F24/F28. acceptance가 없는 사실 수용은 보류. 근거를 전부 펼치는 것과 사람이 더 나은 판단을 하는 것은 다른 일이며(F28), 제시 방식도 평가 대상이다 |
...
| D15 | 단일 writer, read-only 논의자, 실행 sandbox·data-egress 경계 | F05/F16/F17/F19/F24/F26/F27/F31. 실제 OS/adapter 시험 전 안전성 완료 주장 금지. 필수 제한을 적용할 수 없으면 실행 후 경고가 아니라 프로세스 생성 전에 차단. 공급자가 상호작용 데이터를 쓰는지(F31의 기본 동의)도 data-egress 경계에 들어간다 |
...
| D18 | 구독·가용 모델·정족수 preflight, silent fallback 금지 | F04/F05/F06/F17/F18/F19/F25/F26/F29/F30/F31. 실제 entitlement/transport conformance 필요. silent fallback은 정책 전환만이 아니라 플래그 하나(F25)나 실행 모드 강등(F26)으로도 일어남. 남은 한도를 정확히 알 수 없으므로(F29) preflight는 잔여량 추정이 아니라 경로 확인에 기댄다. 구독 경로는 실제로 닫힌 적이 있고(F30), 우리 앱이 공식 CLI를 구동하는 것이 약관상 허용되는지 불명확한 경로도 있다(F31). **경로가 닫히거나 한도를 다 쓰면 그 provider를 구성에서 뺀다 — 유료 API로 전환하지 않는다**(사용자 결정 2026-09-23). provider는 붙였다 뗐다 할 수 있는 adapter이며, 빠진 자리를 다른 모델로 조용히 채우지 않고 구성 축소로 표시한다 |
