...
- 복수 실제 CLI 동시 참여와 실제 합성. 단일 Codex 경로의 첫 응답은 기록했다. 다음은 복수 참여 연결과 provider별 실행 조건·호출 상한을 준비하고 실측하는 단계다(인계 4절 6a–6c).
