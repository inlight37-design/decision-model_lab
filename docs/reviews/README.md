...
가장 최근 기록은 [검토 요청서](2026-09-25-review-request/README.md)다 — 새 컴퓨터 준비 도구, 못 고치는 것, 남은 일의 우선순위, 더 조사할 것을 모아 다른 AI의 검토를 받는다. 그 앞은 [플러그인 점검](2026-09-25-plugin-surface/README.md)이다 — 사용자가 ChatGPT에 GitHub 플러그인을 붙여 카드 이슈에 댓글을 달게 한 뒤, 계정의 플러그인이 앱의 Codex 참여자에게 닿지 않는지 모델 없이 확인했다(지금은 닿지 않음, 보강은 카드 #64). 그 앞은 [#56 검토·병합](2026-09-25-merge-56/README.md)이다 — ChatGPT 웹 세션의 [여러 AI 작업 방식 후속 검토](2026-09-25-workflow-evaluation/README.md)(수퍼바이저 AI의 판단과 실행 관리 프로그램의 분리, 작업·시도·세션 구분, 안전한 인계, 작은 카드 시범)를 출처까지 다시 읽고 병합했다. 제품 코드가 아니라 개발 방식에 관한 제안이고, Projects 행 하나를 영어 원문 기준으로 정정했다. 그 앞은 [#52 검토·병합](2026-09-25-merge-52/README.md)이다 — ChatGPT 웹 세션의 [전체 코드 재검토](2026-09-25-deep-review/README.md)(합성 인용 무결성·저장된 상한 재검사·사건 조회 인덱스·서버 정상 종료)를 확인해 병합했고, 서버를 끄면 돌던 시도가 "결과 모름" 대신 취소·자손 종료 확인으로 기록되는 것을 WSL 모의 서버로 전후 비교했다. 그 앞은 [strict 첫 실제 실행과 실제 중도 취소](2026-09-25-strict-live-run/README.md)다 — 앱이 독립 정족수를 처음 채웠고, Codex가 처음 실제 합성을 했으며, 실행 중인 CLI를 취소했다. 그 앞은 [참여자 문맥 독립성(E2)](2026-09-25-context-independence/README.md)이다 — 두 참여자 계획의 문맥 통로를 좁히고 행동 표식 대조로 C3를 관측했다. 그 앞 실행 경로의 후속은 [#46·#47 검토·병합](2026-09-25-merge-46-47/README.md)이다. [#46 기록](2026-09-25-runtime-review/README.md)의 합성 수명주기 수정과 [#47 감사](2026-09-25-runtime-audit-finish/README.md)의 자료 복원 검사를 함께 main에 넣었고, #47이 원격 미반영으로 남긴 사용량 파서(R1)와 새로 찾은 사용량 칸의 NaN(U1)을 고쳤다. 과거 기록의 완료 상태를 현재 head의 CI로 읽지 않는다.
...
| [`2026-09-25-review-request/`](2026-09-25-review-request/README.md) | claude의 검토 요청서. 새 컴퓨터 준비 도구([SETUP](../SETUP.md)), 못 고치는 것(A1–A7), 해야 할 일의 우선순위(PR #69, 카드 #70–#72 포함), 더 조사할 것(C1–C10), 사용자가 정할 것과 검토 질문 일곱. 결과는 `<YYYY-MM-DD>-review/` 또는 리뷰 카드 댓글로 받는다 |

최신 추가: [`2026-09-25-review-request/`](2026-09-25-review-request/README.md) — PR #52 뒤로 외부 전체 검토를 받지 않은 범위와 새 컴퓨터 준비 도구, 남은 일·한계·조사 목록에 대한 검토 요청(아직 리뷰받지 않음). 그 앞 [`2026-09-25-plugin-surface/`](2026-09-25-plugin-surface/README.md) — 계정 플러그인과 Codex 참여자 표면(모델 없음). 그 앞 [`2026-09-25-merge-56/`](2026-09-25-merge-56/README.md) — #56의 여러 AI 작업 방식 후속 검토를 출처까지 다시 읽고 병합했다. 그 앞 [`2026-09-25-merge-52/`](2026-09-25-merge-52/README.md) — #52의 서버 종료 수정을 사용자 PC(WSL, 모의 참여자)에서 처음 확인했다. 그 앞 [`2026-09-25-strict-live-run/`](2026-09-25-strict-live-run/README.md) — E2 manifest 위에서 앱이 처음으로 독립 정족수를 채웠다. 그 앞 [`2026-09-25-context-independence/`](2026-09-25-context-independence/README.md)는 두 지금 계획의 문맥(C3)을 이 PC·판·30일 범위에서 observed로 기록했다. 앞선 [`Codex 연결 앱 끄기`](2026-09-24-codex-apps-off/README.md)의 "C3 미확인"은 그 당시 계획에 대한 것이다.
