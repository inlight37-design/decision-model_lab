...
5. 다른 세션의 열린 PR과 같은 파일을 고칠 예정이면 **그 브랜치 위에서 시작하거나 병합을 기다린다.** 남의 브랜치에 직접 push하지 않는다. 그 브랜치 위에서 시작해도 **PR은 main을 대상으로 연다.** PR의 대상을 그 브랜치로 하면, 그 브랜치의 PR이 병합될 때 브랜치 자동 삭제로 이 PR이 닫힐 수 있다(2026-09-24 PR #19).
