...
- **AI 세션**은 브랜치에서 작업하고 PR로 제안한다. 사용자가 그 세션에서 명시적으로 지시하지 않으면 main에 직접 push하지 않는다.
...
- **병합 전의 작업은 `NEXT-SESSION.md` 3절에 브랜치 이름으로 적는다.** 그 인계 갱신 자체도 main에 들어가야 웹 세션이 본다 — 3절을 고친 커밋이 병합되기 전이면 웹 세션에게 브랜치 이름을 직접 알려 준다.
