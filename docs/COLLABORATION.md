...
| 쌓인 것 | 왜 쌓였나 | 원칙 | 막는 장치 |
|---|---|---|---|
| 인계 문서가 다섯 번 불었다 | 세션마다 한 일을 덧붙이고 빼는 사람이 없었다 | **살아 있는 문서에는 지금만 적는다.** 한 일은 PR·git·날짜 기록이 갖는다 | 크기 상한, 3절의 "이 판을 들인 PR" 한 줄 |
...
| 문서의 종류와 쓰임이 어긋남 | 날짜 박힌 v0.4 HANDOFF가 살아 있는 목록에 있었고, 지금 따라 하는 절차(V04-01, E2 재관측)는 날짜 기록 폴더에 있다 | **종류는 쓰임을 따른다**(4절) | 도구 사용처 검사의 `PROCEDURES` — 옮기기 전까지의 예외 목록 |
...
5. **대신 뺄 것이 있는가.** 규칙이나 문장을 더하면 낡은 것을 찾아 뺀다. 크기 상한에 걸리면 상한을 올리지 말고 이력·끝난 일·중복을 뺀다. 그래도 올려야 하면 PR 본문에 이유를 적는다.
...
- 날짜 기록, 원장·관측 원본, 사용자가 확정한 것(인계 2절)은 지우지 않는다. 쓰는 곳이 없지만 지울지 확신이 없는 도구는 검사의 `KEPT`에 남기는 이유와 그것을 정할 카드를 적는다.
