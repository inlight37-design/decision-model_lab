...
| 문서의 종류와 쓰임이 어긋남 | 날짜 박힌 v0.4 HANDOFF가 살아 있는 목록에 있었고, 지금 따라 하는 절차(V04-01, E2 재관측)는 날짜 기록 폴더에 있다 | **종류는 쓰임을 따른다**(4절) | 도구 사용처 검사의 `PROCEDURES` — 옮기기 전까지의 예외 목록, 결정 날짜가 지나면 CI 실패 |
