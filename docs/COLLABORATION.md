...
| 종류 | 파일 | 규칙 |
|---|---|---|
| **살아 있는 안내** | 루트의 안내·규칙·인계, 이 문서, 설치 안내, 목록 README들 — 목록은 [`tests/test_research_integrity.py`](../tests/test_research_integrity.py)의 `LIVING_DOCS` 하나가 기준이다 | 항상 현재 기준. 개수 금지, 원장 범위는 실제와 일치(CI) |
...
새 살아 있는 문서를 만들면 `LIVING_DOCS`에 추가한다. **종류는 쓰임을 따른다** — 다시 따라 할 절차는 살아 있는 문서에, 그 시점의 결과는 날짜 기록에 둔다(7절).

## 5. 세션을 끝낼 때

1. 검사를 실행한다 — 명령은 [`NEXT-SESSION.md`](../NEXT-SESSION.md)의 검사 절.
2. **`NEXT-SESSION.md`를 같은 PR 안에서 갱신한다.** 절 구성은 고정이고 CI가 확인한다. **3절에는 진행 중인 일과 "이 판을 들인 PR" 하나만 둔다** — 자기 PR로 바꿔 적는다. 병합 전 브랜치에서도 병합 뒤 main에서도 맞는 말이어야 하므로 "병합했다"고 미리 쓰지 않는다(2026-09-25 외부 검토 R07). 이력은 PR 목록과 git이 갖는다. 이력을 쌓으면 인계가 커지고, 동시에 일하는 세션끼리 같은 줄에서 충돌한다(2026-09-25 PR #68·#69·#74, 줄이기 전 판은 줄인 판의 두 배 반). 통째로 새로 쓰면 이전 판을 `docs/handoff/YYYY-MM-DD-<사유>.md`로 옮긴다.
...
| CI 쌓임 검사([`tests/test_accumulation.py`](../tests/test_accumulation.py)) | 인계·`AGENTS.md`·이 문서가 크기 상한을 넘는 것, 인계 3절에 앞 PR을 줄줄이 적는 것, 쓰는 곳 없는 도구(7절) |
...
2026-09-25 정리(인계 2절 23)에서 본 쓸모없는 것들은 모두 **더하는 사람은 있고 빼는 사람은 없는** 모양으로 쌓였다. 원칙은 그 길마다 하나이고, 기계가 잡을 수 있는 것은 CI(6절의 쌓임 검사)가 잡는다.

| 쌓인 것 | 왜 쌓였나 | 원칙 | 막는 장치 |
|---|---|---|---|
| 인계 문서가 되풀이해 불었다 — 불 때마다 통째로 새로 써서 [보관](handoff/README.md)했고, 마지막에는 줄인 판의 두 배 반이었다 | 세션마다 한 일을 덧붙이고 빼는 사람이 없었다 | **살아 있는 문서에는 지금만 적는다.** 한 일은 PR·git·날짜 기록이 갖는다 | 크기 상한, 3절의 "이 판을 들인 PR" 한 줄 |
| `AGENTS.md`에 실행 이력 서술 | 규칙 파일에 상태를 적었다 | **규칙 파일에는 규칙만.** 상태는 인계 1절, 근거는 기록 링크 | 크기 상한 |
| 같은 목록·같은 경고가 두세 곳에 | 새 기록마다 여러 곳을 손으로 고쳤다 | **한 사실은 한 곳에.** 나머지는 링크만 | 개수·원장 범위 검사, 그 밖은 PR 검토 |
| 끝난 일회성 도구 | 그 도구만 시험하는 테스트와 자기 README가 쓰는 곳처럼 보였다 | **도구는 지금 쓰는 곳이 있어야 남는다** — 앱·`core` 코드, CI, 살아 있는 다른 도구, 살아 있는 문서·절차. 자기 테스트·README·날짜 기록은 쓰는 곳이 아니다 | 도구 사용처 검사 |
| 실험마다 복사한 실행 스크립트(`drive.py`) | 비슷한 일에 앞 실험의 것을 복사했다 | **복사하지 말고 가져다 쓴다.** 두 번째 복사가 필요해지면 공용 명령으로 뺀다(`app.run`처럼) | PR 검토 |
| 설정과 어긋난 규칙(CI가 도는 때, 브랜치 삭제) | 설정·workflow를 바꾸고 문장은 그대로 뒀다 | **규칙은 그것을 지키는 장치와 같은 PR에서 바꾼다** | PR 검토, 6절 표 |
| 문서의 종류와 쓰임이 어긋남 | 날짜 박힌 v0.4 HANDOFF가 살아 있는 목록에 있었고, 지금 따라 하는 절차가 날짜 기록 폴더에 있었다(E2 재관측 명령은 SETUP으로 옮김, V04-01은 남음) | **종류는 쓰임을 따른다**(4절) | 도구 사용처 검사의 `PROCEDURES` — 옮기기 전까지의 예외 목록, 결정 날짜가 지나면 CI 실패 |

**규칙을 더하기 전에.** 규칙도 쌓인다. 새 규칙이나 검사를 더하기 전에 다섯 가지를 확인한다.

1. **실제로 일어난 일이 있는가.** 일어날 수 있는 일만으로는 만들지 않는다. 그 일(PR·날짜)을 이유로 함께 적는다 — 이유가 사라지면 규칙도 뺄 수 있다.
2. **있는 규칙을 고쳐서 되는가.** 되면 새 줄을 만들지 않는다.
3. **기계가 잡을 수 있는가.** 잡을 수 있으면 CI 검사로 만들고 문장은 짧게 둔다. 못 잡으면 누가 언제 보는지 적는다. 사람이 기억해야만 지켜지는 규칙은 오래가지 않았다 — 인계 3절 규칙이 문장으로만 있는 동안 인계가 다시 불었다.
4. **한 곳에만 쓰는가.** 저장소 전반의 규칙은 `AGENTS.md`, 세션 사이의 협업은 이 문서, 지금 단계의 주의는 인계 5절. 다른 곳에서는 링크만 건다.
5. **대신 뺄 것이 있는가.** 규칙이나 문장을 더하면 낡은 것을 찾아 뺀다. 크기 상한에 걸리면 상한을 올리지 말고 이력·끝난 일·중복을 뺀다. 상한이 없는 다른 문서로 옮겨 피하지 않는다 — 옮기는 곳이 그 사실의 집일 때만 옮기고, PR 검토는 옮긴 곳도 본다. 그래도 올려야 하면 PR 본문에 이유를 적는다.

**정리할 때.** 크기 상한이나 도구 검사에 걸렸을 때, 그리고 참여자 관측을 다시 할 때(관측 기록은 30일이 지나면 쓸 수 없어 적어도 한 달에 한 번 온다) 인계의 끝난 줄, 쓰는 곳 없는 도구, 두 곳에 적힌 사실을 한 번 훑는다.

- 날짜 기록이 가리키는 파일을 지우면 기록은 고치지 않고, 그 폴더나 목록 README에 마지막 판의 커밋을 적는다(`git show <커밋>:<경로>`). 그 도구만 시험하던 테스트도 함께 지운다.
- 날짜 기록, 원장·관측 원본, 사용자가 확정한 것(인계 2절)은 지우지 않는다. 쓰는 곳이 없지만 지울지 확신이 없는 도구는 검사의 `KEPT`에 남기는 이유, 그것을 정할 카드, 결정 날짜를 적는다. 예외는 유예이지 보관이 아니다 — 날짜가 지나면 CI가 실패하고, 미루려면 새 날짜와 이유를 PR에 적는다.
