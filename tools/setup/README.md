...
| [`setup-wsl.sh`](setup-wsl.sh) | Ubuntu 쪽 준비. apt 패키지와 공식 Codex·Claude Code를 관측 판으로 설치한다. 설치 스크립트의 SHA-256이 읽어 둔 값과 다르면 멈춘다. `setup.ps1`이 root로 `--apt-only`, 사용자로 `--no-final`을 부른다. 혼자 돌리면 sudo로 전부 한다. `--check`면 보고만 한다 |
