...
최신 관측: [E2 문맥 독립성](../../docs/reviews/2026-09-25-context-independence/README.md)(2026-09-25). 그때 계획 `codex@bba3751a36f3`의 K46(쓰기 `EROFS`·로그인 파일 `EACCES`·`~/.codex` 목록 `EACCES`)과, `c3-claude(-pos)`·`c3-codex(-pos)`의 행동 표식 양성·음성 대조, 모델 없는 [`codex_prompt_input.py`](codex_prompt_input.py)의 입력 렌더링으로 두 계획의 C3를 observed로 적었다. 그 뒤 Codex 계획에 플러그인 끄기가 더해져(`codex@5bed42d05320`, [기록](../../docs/reviews/2026-09-25-codex-plugins-off/README.md)) 재관측(카드 #82)을 기다린다. **관측 기록은 자기 판만 뒷받침한다** — 새 관측은 요약의 `spec.revision`을 기록의 `spec_revision`으로 옮기고, `--keep-session` 같은 probe 변형은 참여자 계획과 다른 판이다. Claude `b1` probe는 참여자와 같은 계획으로 돌며 `argv_changes=[]`로 그것을 확인한다. 옛 계획의 관측(연결 앱을 켠 [첫 K46 확인](../../docs/experiments/w2-isolation/2026-09-24-k46-confirmation/README.md), [연결 앱을 끈 재관측](../../docs/reviews/2026-09-24-codex-apps-off/README.md) 등)은 [리뷰 목록](../../docs/reviews/README.md)에 날짜순으로 있다. 첫 확인이 실제로 돈 판 `codex@8a0128d4c791`은 옛 기록 이름 `discussant-2`에만 대응한다([`core/contract.py`](../../core/contract.py)의 `LEGACY`). CLI와 실제 제공 모델의 일치는 별도 미확인이다.
...
| [`codex_prompt_input.py`](codex_prompt_input.py) | E2. `codex debug prompt-input`으로 모델에 보이는 입력을 모델 없이 렌더링한다. 합성 HOME에서 전역·작업 폴더 AGENTS.md 표식의 양성 대조(기본값·`project_doc_max_bytes=0`·빈 전역 파일)를 하고, `--real-home`(상시 승인 뒤)은 참여자 계획의 `-c` 값·격리·실행 위치로 실제 HOME과 합성 HOME을 비교해 모양·표식·줄 차이의 수만 적는다. 최종 요청 전체가 아니다(도구 schema·exec 전용 옵션 없음). 결과: [E2](../../docs/reviews/2026-09-25-context-independence/README.md) |
...
끝난 일회성 진단 넷 — `cli_boundary.py`(첫 경계 시험), `auth_mounts.py`(N3·K09), `codex_sandbox.py`(K12·K09·K46), 그리고 `app.codex_account`를 부르기만 하던 `codex_account.py`(이제 `python -m app.codex_account`) — 은 2026-09-25에 뺐다. 기록이 가리키는 마지막 판은 main `01cedd3`에 있다(`git show 01cedd3:tools/w2/<파일>`).
