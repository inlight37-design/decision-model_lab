...
plan은 controller와 같은 json/빈 도구/자료 없는 계획 및 별도 stream-json 진단 판을 보인다.
...
LIMITS = {
    "permission_conformance": "insufficient: no tool execution trace in the exact participant result JSON; "
...
def stream_variant(argv: list[str]) -> tuple[list[str], list[str]]:
    """진단 계획만. controller argv를 몰래 바꾸지 않는다."""
    i = argv.index("--output-format")
    argv[i + 1:i + 2] = ["stream-json", "--verbose"]
    return argv, ["--output-format stream-json --verbose (diagnostic only)"]
...
def describe(exact: contract.Plan, diagnostic: contract.Plan, home: str) -> dict:
    return scrub_all({
        ...
        "participant": exact.record(), "diagnostic_stream_json": diagnostic.record(),
        ...
        "note": "No approval or call is performed here. The optional stream-json plan needs its own "
                "approved diagnostic implementation; it is not plain-claude and cannot update the participant record.",
